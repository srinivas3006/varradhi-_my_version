...
void main() {
  ...
  group('Step 9 Reliability: Storage & Startup Resilience (P0-02)', () {
    test('AppState init safely handles empty or corrupted prefs and defaults to guest', () async {
      SharedPreferences.setMockInitialValues({
        'hasOnboarded': true,
        'language': 'Telugu',
  ...
  group('Step 9 Reliability: UGC Upload & Cancellation Propagation (P0-01)', () {
    test('UgcRepository propagates DioException for cancel and status codes', () async {
      final tempDir = Directory.systemTemp.createTempSync('ugc_cancel_test');
      final tempFile = File('${tempDir.path}/test_upload.jpg')..writeAsBytesSync([1, 2, 3]);
  ...
  group('Step 9 Reliability: Feed Response Hardening & Proxy Error Defense (P1-02)', () {
    test('FeedRepository safely handles HTML/non-Map 502 error responses without throwing TypeError', () async {
      final dio = Dio();
    ...
    test('FeedRepository safely skips malformed article items in data list', () async {
      final dio = Dio();
      dio.interceptors.add(
        InterceptorsWrapper(
          onRequest: (options, handler) {
  ...
  group('Step 9 Reliability: Navigation Unmounted Safety (P1-03)', () {
    testWidgets('AppNavigator pushSafe returns null when context is unmounted', (tester) async {
  ...
  group('Step 9 Reliability: Ad Event Queue Bounded Capacity & Deduplication (P2-01)', () {
    setUp(() {
    ...
    test('AdEventQueue ignores empty adId and enqueues valid events', () {
      ...
      queue.enqueue(adId: 'ad-100', eventType: 'impression', placementZone: 'feed');
      queue.enqueue(adId: 'ad-100', eventType: 'click', placementZone: 'feed');
      queue.enqueue(adId: 'ad-200', eventType: 'impression', placementZone: 'feed');
    ...
    test('AdEventQueue enforces maxQueueSize = 50 and drops oldest on overflow', () {
      final queue = AdEventQueue.instance;
      for (int i = 0; i < 60; i++) {
        queue.enqueue(adId: 'ad-$i', eventType: 'impression', placementZone: 'feed');
      }
  ...
  group('Step 9 Reliability: Guest 401 Handling (P2-03)', () {
    test('Guest user receiving 401 does not trigger logout state notification', () async {
      final state = AppState.instance;
  ...
  group('Step 9 Reliability: Deep Link & URI Resilience', () {
    test('Resolves varadhi:// article and category deep links', () {
      final catTarget = NotificationDeepLinkResolver.resolveFromUri('varadhi://category/education');
      expect(catTarget.type, equals(NotificationTargetType.category));
      expect(catTarget.identifier, equals('education'));

      final artTarget = NotificationDeepLinkResolver.resolveFromUri('varadhi://article/telangana-budget-2026');
    ...
    test('Resolves custom scheme article:// and relative paths', () {
      final artTarget = NotificationDeepLinkResolver.resolveFromUri('article://hyderabad-metro-phase-2');
      expect(artTarget.type, equals(NotificationTargetType.article));
      expect(artTarget.identifier, equals('hyderabad-metro-phase-2'));

      final relTarget = NotificationDeepLinkResolver.resolveFromUri('/article/hyderabad-metro-phase-2');
    ...
    test('Resolves screen:// targets with correct auth flags', () {
      final bookmarks = NotificationDeepLinkResolver.resolveFromUri('screen://bookmarks');
      ...
      final settings = NotificationDeepLinkResolver.resolveFromUri('screen://settings');
    ...
    test('Safely handles empty, malformed, and unknown URIs without throwing', () {
      ...
      final malformed = NotificationDeepLinkResolver.resolveFromUri('varadhi://');
      expect(malformed.type, equals(NotificationTargetType.unknown));

      final unknownScheme = NotificationDeepLinkResolver.resolveFromUri('unknownscheme://foo/bar');
      expect(unknownScheme.type, equals(NotificationTargetType.unknown));
    });
  });

  group('Step 9 Reliability: Video Playback Error Handling', () {
    test('NetworkVideoPlaybackController handles empty/invalid URL with status error', () async {
